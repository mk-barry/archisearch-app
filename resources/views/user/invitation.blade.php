...
<head>
    ...
    @php
        use Carbon\Carbon;
        // On formate la date proprement (ex: 15 Avr 2026)
        $deadline = Carbon::parse($event->end_date)->translatedFormat('d M Y');
    @endphp
            ...
            <div class="card-body">
                ...
                <div class="description-box">
                    <p class="desc-text">
                        ...
                        {{ $event->description ?? "L'administration vous invite à soumettre les documents listés ci-dessous avant la date limite indiquée." }}
                    </p>
                    <div class="meta-row">
                        <div class="meta-item">
                        ...
                        <div class="meta-item">
                            ...
                            <strong>{{ count($event->required_docs) }} documents</strong> à soumettre
                        </div>
                    </div>
                </div>
                ...
                <div class="checklist-section">
                    ...
                    {{-- BOUCLE DYNAMIQUE SUR LES DOCUMENTS --}}
                    @foreach($event->required_docs as $index => $docName)
                        <div class="checklist-item">
                            <div class="number-badge">{{ $index + 1 }}</div>
                            <div class="item-name">{{ $docName }}</div>
                            <div class="item-format">PDF, JPG, PNG · max {{ $event->max_file_size ?? 20 }} Mo</div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="security-footer">
                    ...
                    Lien sécurisé · Valide jusqu'au {{ $deadline }}
                </div>
            </div>
        </div>
